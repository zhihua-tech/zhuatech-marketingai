...
package cn.zhuatech.marketingai;
 ...
 */
@SpringBootTest @AutoConfigureMockMvc class MarketingAiApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("本月内容"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanBuildCampaignBrief()throws Exception{mvc.perform(post("/api/ai/campaign/brief").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"product\":\"企业知识库\",\"audience\":\"数字化负责人\",\"channel\":\"公众号\",\"tone\":\"专业克制\",\"containsPerformanceClaim\":true}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.contentAngles.length()").value(3)).andExpect(jsonPath("$.data.reviewChecklist.length()").value(4));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
